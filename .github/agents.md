...
- **Functions deploy timeout**: Remote builds (with `--build-remote true`) compile numpy/scipy on Azure and can take 7-10 minutes. Timeout is set to **600s** to avoid false failures.
- **AI Search region**: Uses **eastus** instead of centralus due to recurring capacity/provisioning issues in centralus.
- **AI Search pre-deployment cleanup**: The deploy workflow detects Search services stuck in "Failed" provisioning state and deletes them before deploying, preventing "Conflict" errors from zombie resources.
- **Mobile UI**: Episode list uses a hamburger menu with slide-out drawer on screens ≤900px. Overlay div must be outside the CSS grid container to avoid breaking layout.
- **Podcast prosody**: Both HOST and EXPERT voices use `-5%` rate for consistent pacing; only EXPERT has `+2%` pitch shift.
