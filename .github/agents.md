...
- **Disabling EasyAuth also wipes the provider registration**: it clears `identityProviders.azureStaticWebApps`, and simply setting `platform.enabled=true` afterwards is not enough — auth would be on with nothing that trusts SWA. Re-linking (unlink then link) is the only supported repair, which is why the deploy workflow now checks both `platform.enabled` and the provider registration before deciding the backend is healthy.
