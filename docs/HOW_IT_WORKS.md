...
Both diagrams on this page are generated by
[`scripts/build-diagrams.py`](../scripts/build-diagrams.py), which emits the SVG
shown here plus an editable [Excalidraw](https://excalidraw.com) source next to
it ([architecture.excalidraw](diagrams/architecture.excalidraw),
[generation-flow.excalidraw](diagrams/generation-flow.excalidraw)). Edit the
coordinate table in the script and re-run it - a built-in layout check fails the
build on overlapping boxes, diagonal connectors, lines routed through a box, or
